...
__all__ = ["DiffusionModelUNet_aniso", "DiffusionModelUNet_aniso_enc"]
    ...
    def __init__(
        ...
        if use_conv:
            ...
            if self.iso_conv_down:    
                ...
                if self.num_channels != self.out_channels:
                    raise ValueError("num_channels and out_channels must be equal when use_conv=False")
                self.op = Pool[Pool.AVG, spatial_dims](kernel_size=2, stride=(2,2,1))
...
class DiffusionModelUNet_aniso(nn.Module):
    """
...
class DiffusionModelUNet_aniso_enc(nn.Module):
    """
    Unet network with timestep embedding and attention mechanisms for conditioning based on
    Rombach et al. "High-Resolution Image Synthesis with Latent Diffusion Models" https://arxiv.org/abs/2112.10752
    and Pinaya et al. "Brain Imaging Generation with Latent Diffusion Models" https://arxiv.org/abs/2209.07162
    ...
    def forward(
        ...
        h = h.view(h.shape[0], h.shape[1], -1).float()
