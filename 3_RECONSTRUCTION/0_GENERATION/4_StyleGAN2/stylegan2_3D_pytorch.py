...
class Trainer():
    def __init__(
        ...
        **kwargs
        ...
        self.fid_dir = Path('./fid')
        self.config_fold = Path('./config')
        self.config_path = Path(f'./config_{name}.json')
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        generated_images = self.generate_truncated(self.GAN.S, self.GAN.G, latents, n_1, n_2, trunc_psi = self.trunc_psi)
        save_image(generated_images, str(f'/workspace/Ablation/ABLATION_PD/StyleGAN2/synth_img/{str(num)}.nii.gz'))
    ...
    def load(self):
        # self.load_config()
        self.GAN = StyleGAN2()
        file_paths = "/workspace/PD_SSL_ZOO/2_DOWNSTREAM/WEIGHTS/4_StyleGAN.pt"

        print(f'continuing from previous epoch')

        self.steps = 60 * self.save_every

        load_data = torch.load(file_paths)
