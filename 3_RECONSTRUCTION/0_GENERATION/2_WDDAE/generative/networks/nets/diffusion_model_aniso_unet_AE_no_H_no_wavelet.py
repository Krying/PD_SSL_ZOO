...
__all__ = ["DiffusionModelUNet_aniso_AE_no_H_no_wavelet", "DiffusionModelEncoder_ansio_no_H_no_wavelet"]
...
class DiffusionModelUNet_aniso_AE_no_H_no_wavelet(nn.Module):
    """
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            h, res_samples = downsample_block(hidden_states=h, temb=emb, cond=cond, context=context)
        ...
        # 5. mid
        h = self.middle_block(hidden_states=h, temb=emb, cond=cond, context=context)
        ...
        for upsample_block in self.up_blocks:
            ...
            h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, temb=emb, cond=cond, context=context)
        ...
        # 7. output block
        if self.multi_res_loss:
            h_1 = self.out1(h_list[-1])
            h_2 = self.out2(h_list[-3])
            h_3 = self.out3(h_list[-4])
            h = [h_1, h_2, h_3]
        else:
            h = self.out1(h)
...
class DiffusionModelEncoder_ansio_no_H_no_wavelet(nn.Module):
    def __init__(
        self,
        spatial_dims: int,
        in_channels: int,
        ...
        self.out5 = nn.Sequential(normalization(num_channels[4]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((1,1,1)), nn.Flatten(), nn.Linear(num_channels[4]*1*1*1, 512))
    ...
    def forward(
        ...
        # latent_list = [output0, output1, output2, output3, output4, output5]
        latent_list = self.out5(h)
