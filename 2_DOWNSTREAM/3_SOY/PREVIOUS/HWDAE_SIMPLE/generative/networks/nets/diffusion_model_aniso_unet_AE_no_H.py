...
__all__ = ["DiffusionModelUNet_aniso_AE_no_H", "DiffusionModelEncoder_ansio_no_H"]
...
class DiffusionModelUNet_aniso_AE_no_H(nn.Module):
    """
...
class DiffusionModelEncoder_ansio_no_H(nn.Module):
    def __init__(
        self,
        spatial_dims: int,
        in_channels: int,
        ...
        # For just DAE, we only need for last 512 vector
        ...
        # self.out5 = nn.Sequential(normalization(num_channels[4]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((2,2,2)), nn.Flatten(), nn.Linear(num_channels[4]*2*2*2, 512))
