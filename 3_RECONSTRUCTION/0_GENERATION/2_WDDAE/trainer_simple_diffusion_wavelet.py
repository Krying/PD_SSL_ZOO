...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape):
        ...
        empty = torch.randn(shape, device = self.device)
        coeffs3 = ptwt.wavedec3(empty, pywt.Wavelet('haar'), level=1, mode='zero')
        
        coeffs3[0] = img[:,0:1,:,:,:]
        coeffs3[1]['aad'] = img[:,1:2,:,:,:]
        coeffs3[1]['ada'] = img[:,2:3,:,:,:]
        coeffs3[1]['add'] = img[:,3:4,:,:,:]
        coeffs3[1]['daa'] = img[:,4:5,:,:,:]
        coeffs3[1]['dad'] = img[:,5:6,:,:,:]
        coeffs3[1]['dda'] = img[:,6:7,:,:,:]
        coeffs3[1]['ddd'] = img[:,7:8,:,:,:]
        
        img = ptwt.waverec3(coeffs3, pywt.Wavelet("haar"))
        return img

    @torch.no_grad()
    def sample(self, batch_size = 1):
        return self.p_sample_loop((1, 8, 96, 96, 48))
    ...
    def p_losses(self, x_start, times, noise = None):
        noise = default(noise, lambda: torch.randn_like(x_start)) #B*8*96*96*48
...
class Trainer(object):
    ...
    def load(self, milestone):
        ...
        data = torch.load(f'/workspace/DIF_DDAE_PET/results/256_128_128_256_512_2e-05_model_wddpm/model-{milestone}.pt', map_location=device)
    ...
    def load_for_generation(self):
        accelerator = self.accelerator
        device = accelerator.device

        data = torch.load(f'/workspace/PD_SSL_ZOO/2_DOWNSTREAM/WEIGHTS/2_WDDAE.pt', map_location=device)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    coeffs3 = ptwt.wavedec3(data, pywt.Wavelet('haar'), level=1, mode='zero')
                    data = torch.cat((coeffs3[0], 
                                      coeffs3[1]['aad'], 
                                      coeffs3[1]['ada'], 
                                      coeffs3[1]['add'], 
                                      coeffs3[1]['daa'], 
    ...
    def generation(self, num_iter):
        ...
        for i in range(num_iter):
            os.makedirs("/workspace/PD_SSL_ZOO/3_RECONSTRUCTION/0_GENERATION/2_WDDAE/results", exist_ok=True)
            with torch.inference_mode():
                image_pred = self.ema.ema_model.sample(batch_size=1)
                image_pred.clamp_(0.,1.)
                pred_img = image_pred.cpu().detach().numpy().transpose(0,4,3,2,1).squeeze()
                save_pred = sitk.GetImageFromArray(pred_img)
                sitk.WriteImage(save_pred, f"/workspace/PD_SSL_ZOO/3_RECONSTRUCTION/0_GENERATION/2_WDDAE/results/{i}.nii.gz")
