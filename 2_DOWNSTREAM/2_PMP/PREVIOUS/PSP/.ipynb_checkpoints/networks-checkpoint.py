...
class GradualStyleBlock_3D(Module):
    ...
    def forward(self, x):
        # print(f"map2style_first {x.shape}")
        x = self.convs(x)
        # print(f"map2style_last {x.shape}")
...
class GradualStyleEncoder_3D(Module):
    def __init__(self, num_layers = 50):
        ...
        self.input_layer = Sequential(Conv3d(1, 64, (3, 3, 3), 1, 1, bias=False), #image를 바꿔주는 것
                                      BatchNorm3d(64),
                                      PReLU(64))
        
        # self.input_layer = Sequential(Conv3d(1, 128, (3, 3, 3), 1, 1, bias=False), #image를 바꿔주는 것
        #                               BatchNorm3d(128),
        #                               PReLU(128))
        modules = []
        for block in blocks:
            for bottleneck in block:
                modules.append(unit_module(bottleneck.in_channel,
                                           bottleneck.depth,
                                           bottleneck.stride))
        self.body = Sequential(*modules) #파란색, 주황색 레이어

        self.styles = nn.ModuleList()
        self.style_count = 12 #64 VERSION || 10 -> 128 VERSION
        self.coarse_ind = 4
        self.middle_ind = 8

        for i in range(self.style_count):
            if i < self.coarse_ind:
                # style = GradualStyleBlock_3D(512, 512, 16)
                style = GradualStyleBlock_3D(512, 512, 12) #*2/3 #num_pol
            elif i < self.middle_ind:
                # style = GradualStyleBlock_3D(512, 512, 32)
                style = GradualStyleBlock_3D(512, 512, 24)
            else:
                # style = GradualStyleBlock_3D(512, 512, 64)
        ...
        self.linear = nn.Linear(512*12,1)
    ...
    def forward(self, x):
        ...
        # print(f"first_image_shape {x.shape}")

        latents = []
        modulelist = list(self.body._modules.values())
        for i, l in enumerate(modulelist):
            x = l(x)
            # print(f"{i}th main shape : {x.shape}")
            if i == 6:
                c1 = x #torch.Size([1, 128, 48, 48, 24])
                # print(f"c1_shape : {c1.shape}")
            elif i == 20:
                c2 = x #torch.Size([1, 256, 24, 24, 12])
                # print(f"c2_shape : {c2.shape}")
            elif i == 23:
            # elif i == 49:
                c3 = x #torch.Size([1, 512, 12, 12, 6])
                # print(f"c3_shape : {c3.shape}")
...
class pSp(nn.Module):
	...
	def __init__(self):
		...
		self.n_styles = 12 #64 VERSION || 10 -> 128 VERSION
