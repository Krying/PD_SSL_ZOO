...
def parse_option():
    parser = argparse.ArgumentParser("DisAE pre-training script", add_help=False)
    ...
    parser.add_argument("--log_dir", default='/workspace/PD_SSL_ZOO/UPSTREAM/5_DisAE/output', help="path where to tensorboard log")
...
def train_one_epoch(args, model, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (img, mask) in enumerate(data_loader):
        ...
        img_lowres = F.interpolate(
            img_noisy, size=(int(192 / args.interpolate), int(192 / args.interpolate), int(96 / args.interpolate))
        )
        img_resam = F.interpolate(img_lowres, size=(192, 192, 96))
        loss1, _, _ = model(img_resam, mask, img)
        mask = mask.cuda(non_blocking=True)

        loss = loss1 / args.accumulate_step
...
def validate(data_loader, model, epoch):
    ...
    for idx, (img, mask) in enumerate(data_loader):
        ...
        img_resam = F.interpolate(img_lowres, size=(192, 192, 96))
        loss1, img_recon, _ = model(img_resam, mask, img)
        loss = loss1
