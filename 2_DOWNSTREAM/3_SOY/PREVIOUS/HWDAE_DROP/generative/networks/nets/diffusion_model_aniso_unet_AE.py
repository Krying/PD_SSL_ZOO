...
class DiffusionModelEncoder_ansio(nn.Module):
    def __init__(
        ...
        # self.out1 = nn.Sequential(normalization(num_channels[1]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((6,6,3)), nn.Flatten(), nn.Linear(num_channels[1]*6*6*3, 512))
        # self.out2 = nn.Sequential(normalization(num_channels[2]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((4,4,2)), nn.Flatten(), nn.Linear(num_channels[2]*4*4*2, 512))
